...
from switchboard.config_browser_widget import ConfigBrowserWidget
from switchboard.credential_store import CredentialStore
...
class SwitchboardDialog(QtCore.QObject):
    ...
    def __init__(self, script_manager):
        ...
        self.device_manager.signal_device_removed.connect(
            self.on_device_removed)

        # 자동 활성화 상태 변수
        self.is_activating_config = False
        self.devices_to_connect_for_activation = []
        ...
        # Config Browser 추가
        self.setup_config_browser()

        # QTimer for showing device status updates
        self.device_status_timer = QtCore.QTimer(self.window)
        self.device_status_timer.setInterval(1000)

        # P4
        self.p4_utils = None

    def setup_config_browser(self):
        """Config 브라우저를 설정합니다"""
        # Config 브라우저 위젯 생성
    ...
    def _try_change_address(self):
        new_address = self.window.le_ip_address.text()
        old_address = SETTINGS.ADDRESS.get_value()

        if not SETTINGS.ADDRESS.possible_values.__contains__(new_address):
            SETTINGS.ADDRESS.possible_values.append(new_address)

        SETTINGS.ADDRESS.update_value(new_address)
        SETTINGS.save()
        ...
        if self.osc_restart_check(old_address):
            ...
            if old_address != new_address:
                LOGGER.info(f"Updated address to {new_address}")
        else:
    ...
    def set_current_config(self, config_path):
        ...
        # Config 브라우저 업데이트
        if hasattr(self, 'config_browser'):
            self.config_browser.highlight_current_config()
    ...
    def device_added(self, device):
        ...
        device.device_qt_handler.signal_device_status_changed.connect(self.on_device_status_changed_for_activation, QtCore.Qt.QueuedConnection)
        ...
        # Connect device status signals if they exist
        try:
            device.signal_device_status_changed.connect(
                self.on_device_status_changed)
        except AttributeError:
            # Some devices (like nDisplay) don't have these signals
            pass
            
        try:
            device.signal_device_status_message.connect(
                self.on_device_status_message)
        except AttributeError:
            # Some devices don't have status message signal
            pass
    ...
    def device_widget_open(self, device_widget):
        """
        When the open button is clicked for a device
        """
        device = self.device_manager.device_with_hash(device_widget.device_hash)
        
        # nDisplay 디바이스의 경우 map_name을 전달
        if hasattr(device, 'device_type') and device.device_type == 'nDisplay':
            # CONFIG에서 현재 맵 이름을 가져옴
            map_name = CONFIG.CURRENT_LEVEL if CONFIG.CURRENT_LEVEL else ""
            device.launch(map_name)
        else:
            device.launch()
    ...
    def on_device_status_changed(self, device, new_status, old_status):
        # Update the device widget
        self.device_list_widget.on_device_status_changed(
            device, new_status, old_status)

        self.update_connect_and_open_button_states()

    #
    # CONFIG ACTIVATION SEQUENCE
    #

    def activate_config(self, config_path: Path):
        """ Config를 로드, 연결, 실행하는 활성화 시퀀스를 시작합니다 """
        LOGGER.info(f"Activating config: {config_path.name}")
        # 1. Config 로드
        self.set_current_config(config_path)

        # 2. 연결 단계 시작 (UI 업데이트 후)
        QtCore.QTimer.singleShot(100, self._initiate_connection_for_activation)

    def _initiate_connection_for_activation(self):
        """ 활성화 시퀀스의 연결 단계를 시작합니다 """
        from switchboard.devices.device_base import DeviceStatus
        
        # 모든 디바이스를 연결 대상에 포함 (nDisplay 포함)
        self.devices_to_connect_for_activation = [
            d for d in self.device_manager.devices() if d.status < DeviceStatus.OPEN
        ]

        if not self.devices_to_connect_for_activation:
            LOGGER.info("No devices to connect. Proceeding to launch.")
            self._launch_after_activation()
            return

        LOGGER.info(f"Connecting to {len(self.devices_to_connect_for_activation)} devices...")
        self.is_activating_config = True
        self.connect_all_button_clicked(True)
        
        # 연결 시작 후 3초 후에 실행 단계로 진행
        QtCore.QTimer.singleShot(3000, self._launch_after_activation)

    def _launch_after_activation(self):
        """ 활성화 시퀀스의 실행 단계를 시작합니다 """
        from switchboard.devices.device_base import DeviceStatus
        
        LOGGER.info("Launching all devices...")
        
        # nDisplay 디바이스들을 직접 실행
        for device in self.device_manager.devices():
            if hasattr(device, 'device_type') and device.device_type == 'nDisplay':
                LOGGER.info(f"Directly launching nDisplay device {device.name}")
                
                # nDisplay 디바이스는 CLOSED 상태에서도 실행 가능
                if device.status < DeviceStatus.CLOSED:
                    LOGGER.warning(f"nDisplay device {device.name} is not connected. Status: {device.status}")
                    continue
                
                # device_widget_open 메서드를 사용하여 실행
                try:
                    self.device_widget_open(device.widget)
                    LOGGER.info(f"Successfully launched nDisplay device {device.name}")
                except Exception as e:
                    LOGGER.error(f"Failed to launch nDisplay device {device.name}: {e}")
        
        # 다른 디바이스들은 기존 방식으로 실행
        other_devices = [d for d in self.device_manager.devices() 
                        if not (hasattr(d, 'device_type') and d.device_type == 'nDisplay')]
        
        if other_devices:
            LOGGER.info(f"Launching {len(other_devices)} other devices...")
            self.launch_all_button_clicked(True)

    def on_device_status_changed_for_activation(self, device, new_status, old_status):
        """ 자동 활성화 중 디바이스 상태 변경을 처리합니다 """
        from switchboard.devices.device_base import DeviceStatus
        
        LOGGER.debug(f"on_device_status_changed_for_activation: {device.name} {old_status} -> {new_status}, is_activating: {self.is_activating_config}")
        
        if self.is_activating_config:
            if device in self.devices_to_connect_for_activation:
                LOGGER.debug(f"Device {device.name} is in connection list")
                # nDisplay 디바이스는 CLOSED 상태에서도 연결 완료로 간주
                if hasattr(device, 'device_type') and device.device_type == 'nDisplay':
                    if new_status >= DeviceStatus.CLOSED:
                        LOGGER.info(f"nDisplay device {device.name} finished connecting with status: {new_status}")
                        self.devices_to_connect_for_activation.remove(device)
                        LOGGER.debug(f"Remaining devices to connect: {len(self.devices_to_connect_for_activation)}")
                else:
                    # 다른 디바이스는 OPEN 이상 상태에서 연결 완료로 간주
                    if new_status >= DeviceStatus.OPEN:
                        LOGGER.info(f"Device {device.name} finished connecting with status: {new_status}")
                        self.devices_to_connect_for_activation.remove(device)
                        LOGGER.debug(f"Remaining devices to connect: {len(self.devices_to_connect_for_activation)}")
            
            # 모든 디바이스가 연결 완료되었는지 확인
            if not self.devices_to_connect_for_activation:
                self.is_activating_config = False
                LOGGER.info("All devices finished connecting. Proceeding to launch.")
                QtCore.QTimer.singleShot(100, self._launch_after_activation)
