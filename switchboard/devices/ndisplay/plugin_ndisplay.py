...
class DevicenDisplay(DeviceUnreal):
    ...
    csettings = {
        ...
        'ndisplay_config_temp_path': StringSetting(
            attr_name="ndisplay_config_temp_path",
            nice_name="nDisplay Config Temp Path",
            value="%TEMP%/ndisplay",
            tool_tip="Temporary path where nDisplay config files are stored during execution. Use %TEMP% for system temp folder.",
            category="General Settings",
        ),
    }
    ...
    def launch(self, map_name):
        ...
        nodes = [
            ...
            if isinstance(dev, DevicenDisplay)]
        ...
        # Get config temp path from settings
        config_temp_path = DevicenDisplay.csettings['ndisplay_config_temp_path'].get_value(self.name)
        # Replace %TEMP% with actual temp path if needed
        if '%TEMP%' in config_temp_path:
            import tempfile
            config_temp_path = config_temp_path.replace('%TEMP%', tempfile.gettempdir())
        
        # Create config directory if it doesn't exist
        os.makedirs(config_temp_path, exist_ok=True)
        
        cfg_destination = f"{config_temp_path}/%RANDOM%{cfg_ext}"
